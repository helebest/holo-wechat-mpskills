...
def inspect_html_draft(
    ...
    """
    Inspect an HTML draft without calling WeChat APIs.

    Returns metadata used by the CLI dry-run path.
    """
    ...
    return {
        ...
        "local_images": [local_path for _, local_path in local_images],
...
def submit_html_draft(
    ...
    # 初始化客户端（放在本地校验之后，便于 dry-run 复用校验逻辑）
    if client is None:
        client = WeChatClient()
