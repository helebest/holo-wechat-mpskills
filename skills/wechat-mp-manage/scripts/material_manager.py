...
class MaterialManager:
    ...
    def _validate_file(self, file_path: str, media_type: MaterialType) -> None:
        """验证文件是否符合要求"""
        path = Path(file_path)
        ...
        if size > limit:
            raise ValueError(
                f"文件大小 {size / 1024 / 1024:.2f}MB 超过限制 {limit / 1024 / 1024:.2f}MB"
            )
    ...
    def upload_permanent(
        ...
        if media_type == "video":
            ...
            description = {"title": title, "introduction": introduction or ""}
            import json

            extra_data["description"] = json.dumps(description)

        result = self.client.upload_file(
            endpoint, file_path, file_field="media", extra_data=extra_data
        )
    ...
    def upload_article_image(self, file_path: str) -> str:
        ...
        result = self.client.upload_file("/cgi-bin/media/uploadimg", file_path, file_field="media")
    ...
    def get_material(self, media_id: str) -> Dict[str, Any]:
        ...
        result = self.client.post(
            "/cgi-bin/material/get_material", json_data={"media_id": media_id}
        )
        return result

    def download_material(self, media_id: str, save_path: Optional[str] = None) -> bytes:
        """
        下载永久素材文件
    ...
    def delete_material(self, media_id: str) -> bool:
        ...
        Returns:
            是否删除成功
        """
        self.client.post("/cgi-bin/material/del_material", json_data={"media_id": media_id})
        return True
    ...
    def list_materials(
        self, media_type: MaterialType, offset: int = 0, count: int = 20
    ) -> Dict[str, Any]:
        """
        获取永久素材列表
    ...
    def upload_temporary(self, media_type: MaterialType, file_path: str) -> Dict[str, Any]:
        """
        上传临时素材（有效期 3 天）
        ...
        result = self.client.upload_file(
            "/cgi-bin/media/upload", file_path, file_field="media", extra_data={"type": media_type}
        )
    ...
    def get_temporary(self, media_id: str, save_path: Optional[str] = None) -> bytes:
        """
        获取临时素材
        ...
        Returns:
            文件二进制内容
        """
        content = self.client.download_file("/cgi-bin/media/get", params={"media_id": media_id})
    ...
    def get_hd_voice(self, media_id: str, save_path: Optional[str] = None) -> bytes:
        """
        获取高清语音素材
        ...
        """
        content = self.client.download_file(
            "/cgi-bin/media/get/jssdk", params={"media_id": media_id}
        )
...
# 便捷函数
def create_material_manager(client: Optional[WeChatClient] = None) -> MaterialManager:
    """创建素材管理器实例"""
    if client is None:
        client = WeChatClient()
